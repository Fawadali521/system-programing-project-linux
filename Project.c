...
//size
struct linux_dirent { // define struct for inode number offset name and directory length.
    long           d_ino;
    off_t          d_off;
...
long long countlength(long long i) // to count the length of directory
{
	long long count=0;
	while(i!=0)
	{
...
void getdate(unsigned long date) // define get date function to display date
{
	...
	write(1,top,strlen(top)); // display date
	write(1," ",1);
}
void gethours(unsigned long hours) // display the hours
{
	...
	write(1,top,strlen(top));
}
void getminute(unsigned long minute) // display minutes
{
...
void getmonth(unsigned long month) // display months
{
	if (month==1)
		write(1,"Jan",3);
	else if (month==2)
...
int checkleap(unsigned long year) // check the leap year
{
	if ( year%400 == 0)
    	return 1;
  else if ( year%100 == 0)
...
void getmodifiedtime(unsigned long epoch) // if leap year occurs or not it checks then it modifies the time
{
	epoch+=19800;
    ...
    if (ch==0)
    	day++;
    getmonth(month); // call months
    getdate(day); // call date
    gethours(hour); // call hours
    write(1,":",1);
    getminute(minute); // call minutes
}

char* myitoa(long long i, char b[]){  // show the name of name of the group or group id
    char const digit[] = "0123456789";
    char* p = b;
    if(i<0){
        *p++ = '-';
...
void float_to_string(float f, char r[])  // convert any float to strings
{
...
void get_name(int uid) // print the name of the user
{
...
int mystrlen( char *s ) //show length of the string
{
    char *start;
    start = s;
    while( *s != 0 )
...
char* mystrcat(char *s1, const char *s2) // show the contents of the string
{
...
void getnumoflinks(struct stat fileStat) // show number of links
{
...
void getfilesize(struct stat fileStat) // show size of the file
{
...
void getfilesize_with_h(struct stat fileStat,int block,long long blocks) // show hidden files size
{
	float filesize;
	...
	char ans[h+1];
	float_to_string(filesize,ans); // call float to string used above
...
void printinginfo(struct stat fileStat,char **argv,int direct,int harg) // display information in long format of all files and directories
{
...
void read_link(char *path,struct stat sb) // read number of symbolic links
{
...
void listdir(char **argv,int direct,int argl,int arga,int argh) // display list of directories
{
	int fd, nread;
	long long block=0;
	char buf[BUF_SIZE];
	...
	for ( ; ; )
	{
		nread = syscall(SYS_getdents, fd, buf, BUF_SIZE); // a program request a service from the kernel
		if (nread == -1)
		{
			perror("getdents");
			_exit(1);
...
int main(int args,char **argv) //start main function with arguments double pointer is use to store the address of the second pointer
{
	if(args<2)	// they show the error if the args argument is less then two.
	{
	...
	if ((args==2))
	{
		if((fileStat.st_mode & S_IFMT)==S_IFREG) // to consider only the bits involved to determine the file type(S_IFMT). (IF_REG) Regular File
		{
       		write(1,argv[direct],(size_t)mystrlen(argv[direct])); // stdout the files
			write(1,"\n",1);
		}
		else if((fileStat.st_mode & S_IFMT)==S_IFDIR) //Same work as S_ISDIR = S_IFDIR
		{
			listdir(argv,direct,0,0,0);	//list directory within thr current directory
		}
	...
	else if(args==3) // start the ls -argument(-a -lh -la -l)
	{
		if ((mystrlen(argv[arg])==2 && argv[arg][1]=='l')) // start -l
		{
			...
			else if((fileStat.st_mode & S_IFMT)==S_IFLNK)// show long format
			{
				printinginfo(fileStat,argv,direct,0);
				write(1,"\033[1m\x1b[36m",mystrlen("\033[1m\x1b[36m")); // print length and size of buffer
				write(1,argv[direct],(size_t)mystrlen(argv[direct]));
				write(1,"\x1b[0m",mystrlen("\x1b[0m"));
				write(1," -> ",4);
				read_link(argv[direct],fileStat);	//use to print resolved symbolic links
				write(1,"\n",1);
			}
		}
		else if ((mystrlen(argv[arg])==2 && argv[arg][1]=='a')) // start -a show the list with hidden files
		{
			if((fileStat.st_mode & S_IFMT)==S_IFREG)
			{
	       			write(1,argv[direct],(size_t)mystrlen(argv[direct]));
		...
		else if ((mystrlen(argv[arg])==2 && argv[arg][1]=='h')) // start -h display file size in easy to read format
		{
			if((fileStat.st_mode & S_IFMT)==S_IFREG)
			{
	       			write(1,argv[direct],(size_t)mystrlen(argv[direct]));
		...
		else if((mystrlen(argv[arg])==3) && ((argv[arg][1]=='a' && argv[arg][2]=='l') || (argv[arg][1]=='l' && argv[arg][2]=='a'))) // start la or al show hidden files in long format
		{
			if((fileStat.st_mode & S_IFMT)==S_IFREG)
			{
				printinginfo(fileStat,argv,direct,0);
		...
		else if((mystrlen(argv[arg])==3) && ((argv[arg][1]=='h' && argv[arg][2]=='l') || (argv[arg][1]=='l' && argv[arg][2]=='h'))) //start lh or hl display in easy to read format
		{
			if((fileStat.st_mode & S_IFMT)==S_IFREG)
			{
